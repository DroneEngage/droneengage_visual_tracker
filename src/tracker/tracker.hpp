...
// Delay in processing OnTrack by the unit is high, so sending a full rate
// messages is not needed especially that mavlink module has its own timing and
// discards messsages with small timespan. The reason I dont skip the tracking
...
public:
  CTracker(CCallBack_Tracker *callback_tracker)
      : m_valid_track(false), m_callback_tracker(callback_tracker),
        m_output_video_path(""), m_output_video_active(false){
...
protected:
   ...
   * (0,0) top left
   */
  inline float revScaleX(const float &x) const { return (x / m_image_width); };

  inline float revScaleY(const float &y) const { return (y / m_image_height); };
...
private:
  ...
  const std::string trackerTypes[8] = {"BOOSTING",   "MIL",    "KCF",   "TLD",
