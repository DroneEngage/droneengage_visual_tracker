...
// Original rule intent: FRAMES_TO_SKIP_BETWEEN_MESSAGES >
// FRAMES_TO_SKIP_BETWEEN_TRACK_PROCESS
static_assert(FRAMES_TO_SKIP_BETWEEN_MESSAGES >
                  FRAMES_TO_SKIP_BETWEEN_TRACK_PROCESS,
              "FRAMES_TO_SKIP_BETWEEN_MESSAGES must be greater than "
              "FRAMES_TO_SKIP_BETWEEN_TRACK_PROCESS");

bool CTracker::initTargetVirtualVideoDevice(
...
bool CTracker::init(const enum ENUM_TRACKER_TYPE tracker_type,
  ...
  if (frames_to_skip_between_messages < frame_to_skip_between_track_process) {
    std::cout << _ERROR_CONSOLE_BOLD_TEXT_
              ...
              << " frames_to_skip_between_messages should be larger than "
  ...
  if (!video_capture.isOpened()) {
    ...
    std::cout << _ERROR_CONSOLE_BOLD_TEXT_
              << "FATAL ERROR:" << _BK_RED_WHITE_TEXT_
              << " could not open camera at " << _ERROR_CONSOLE_TEXT_
              << video_path << _NORMAL_CONSOLE_TEXT_ << std::endl;
  ...
  video_capture.set(cv::CAP_PROP_FOURCC,
  ...
  if (m_image_width == 0 || m_image_height == 0) {
    std::cerr << _ERROR_CONSOLE_BOLD_TEXT_
              << "Error: Failed to get camera resolution from " << video_path
              << _NORMAL_CONSOLE_TEXT_ << std::endl;
  ...
  std::cout << _SUCCESS_CONSOLE_BOLD_TEXT_
            << "Video Capture:" << _LOG_CONSOLE_BOLD_TEXT << video_path
            << _NORMAL_CONSOLE_TEXT_ << std::endl;
  ...
  std::cout << _SUCCESS_CONSOLE_BOLD_TEXT_ << "Tracker:" << getActiveTracker()
            << _SUCCESS_CONSOLE_TEXT_ << " has been initialized."
            << _NORMAL_CONSOLE_TEXT_ << std::endl;
    ...
    for (unsigned int i = 0; i < m_buffer_count; ++i) {
      if (m_buffers[i].start != MAP_FAILED && m_buffers[i].start != nullptr) {
        if (munmap(m_buffers[i].start, m_buffers[i].length) == -1) {
          std::cerr << "Error: Failed to munmap buffer " << i << ": "
...
void CTracker::trackRect(const float x, const float y, const float w,
  ...
  m_framesThread =
      std::thread([x, y, w, h, this]() { this->track2Rect(x, y, w, h); });
}
...
void CTracker::track(const float x, const float y, const float radius) {
  ...
  std::cout << _INFO_CONSOLE_BOLD_TEXT << "X:" << _LOG_CONSOLE_TEXT
            << std::to_string(x) << _INFO_CONSOLE_BOLD_TEXT
            << ",Y:" << _LOG_CONSOLE_TEXT << std::to_string(y)
            << _INFO_CONSOLE_BOLD_TEXT << ",R:" << _LOG_CONSOLE_TEXT
            << std::to_string(radius) << _NORMAL_CONSOLE_TEXT_ << std::endl;
  ...
  m_framesThread = std::thread(
      [x, y, radius, this]() { this->track2Rect(x, y, radius, radius); });
}

void CTracker::track2Rect(const float x, const float y, const float w,
                          const float h) {
  ...
  if (!video_capture.isOpened()) {
    std::cerr << _ERROR_CONSOLE_BOLD_TEXT_
              << "FATAL ERROR:" << _INFO_CONSOLE_TEXT
              << " could not open camera at " << _ERROR_CONSOLE_TEXT_
              << m_video_path << _NORMAL_CONSOLE_TEXT_ << std::endl;
    return;
  }
  ...
  video_capture >> frame;
  if (frame.empty()) {
    std::cerr << _ERROR_CONSOLE_BOLD_TEXT_ << "ERROR:" << _INFO_CONSOLE_TEXT
              << " First frame is empty from " << _ERROR_CONSOLE_TEXT_
              << m_video_path << _NORMAL_CONSOLE_TEXT_ << std::endl;
    return;
  }
  ...
  // Clamp coordinates within frame boundaries using std::clamp for conciseness
  // and safety Assuming radius is the side length, so bbox_x + radius should be
  // <= width
  float scaled_x =
      std::clamp(x * m_image_width, 0.0f, m_image_width - scaled_width);
  float scaled_y =
      std::clamp(y * m_image_height, 0.0f, m_image_height - scaled_height);
  ...
  if (m_is_tracking_active_initial) {
    ...
    if (m_callback_tracker)
      m_callback_tracker->onTrackStatusChanged(
          TrackingTarget_STATUS_TRACKING_DETECTED);
  } else {
    if (m_callback_tracker)
      m_callback_tracker->onTrackStatusChanged(
          TrackingTarget_STATUS_TRACKING_STOPPED);
  ...
  bool current_valid_track_status =
      false; // Initialize to false, assuming no track initially
  ...
  while (m_process) {
    ...
    {
      const bool should_skip_track_process =
          (frame_counter % m_frame_to_skip_between_track_process) != 0;

      // Update tracking status only when processed ---
      if (!should_skip_track_process) {

        track_success = m_islegacy ? m_legacy_tracker->update(frame, bbox_2d)
                                   : m_tracker->update(frame, bbox);
        ...
        if (m_callback_tracker) {
          if (track_success != current_valid_track_status) {
            ...
            m_callback_tracker->onTrackStatusChanged(
                track_success ? TrackingTarget_STATUS_TRACKING_DETECTED
                              : TrackingTarget_STATUS_TRACKING_LOST);
          }
        }
      }
      ...
      if (track_success) {
        ...
        if (m_islegacy)
          cv::rectangle(frame, bbox_2d, cv::Scalar(0, 0, 200), 2, 1);
        else
          cv::rectangle(frame, bbox, cv::Scalar(0, 0, 200), 2, 1);

        const bool should_skip_message =
            (frame_counter % m_frames_to_skip_between_messages) != 0;
    ...
    if (m_virtual_device_opened) {
      ...
      if (yuv_frame.isContinuous() &&
        ...
        ssize_t bytes_written =
            write(m_video_fd, yuv_frame.data, m_yuv_frame_size);
        if (bytes_written < 0) {
          ...
          if (errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            // Consider adding a counter for repeated warnings to avoid spamming
            // console.
            std::cerr << "Warning: Virtual device " << m_output_video_path
                      << " buffer full? Try reading from it." << std::endl;
            // No `continue` here, we still want to process the frame and
            // display if needed. The next write might succeed if the reader
            // catches up.
          }
          // For other severe errors, consider setting m_process = false;
        }
      } else {
        // This indicates a critical error in setup or understanding of
        // m_yuv_frame_size/conversion.
        std::cerr << "Fatal Error: YUV frame is not continuous or has "
                     "unexpected size ("
                  << yuv_frame.total() * yuv_frame.elemSize() << " vs "
                  << m_yuv_frame_size
                  << "). Cannot write to V4L2 device. Stopping stream."
                  << std::endl;
    ...
    if (elapsed_time < target_frame_time_ms) {
      ...
      std::cout << "Elapsed: " << elapsed_time.count()
                << "ms, Sleeping for: " << sleep_for.count() << "ms"
                << std::endl;
#endif
    }
#ifdef DDEBUG
    else {
      std::cout << _INFO_CONSOLE_BOLD_TEXT << "Warning: Frame processing took "
                << _LOG_CONSOLE_BOLD_TEXT << elapsed_time.count() << "ms "
                << _INFO_CONSOLE_BOLD_TEXT << ", exceeding target "
                << _LOG_CONSOLE_BOLD_TEXT << target_frame_time_ms.count()
                << "ms." << _INFO_CONSOLE_BOLD_TEXT
                << " Cannot maintain 30 FPS." << _NORMAL_CONSOLE_TEXT_
                << std::endl;
    }
#endif
    ...
    if (frame_counter % 10 == 0) {
      std::cout << _INFO_CONSOLE_BOLD_TEXT
                << "Elapsed: " << _LOG_CONSOLE_BOLD_TEXT << elapsed_time.count()
                << "ms" << std::endl;
    }
#endif
